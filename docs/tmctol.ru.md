...
- Гиперболический ценовой пол через механику XYK: `P_floor = k/(R_native + S_sold)²`, где `k = R_native × R_foreign`
...
P_floor = const(R_native, R_foreign)
         (пол неизменен при постоянных резервах)
...
Скорость:                dP_floor/dt ∝ (скорость_сжигания × R_foreign)/(R_native)⁴
```
