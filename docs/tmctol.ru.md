...
- XYK-модель цены с двумя ориентирами: спотовая цена пула `P_xyk = R_foreign/R_native` и стресс-огибающая пола `P_stress(x) = k/(R_native + x)²`, где `k = R_native × R_foreign`
...
- `Perbill`: Отношение в миллиардных долях (10⁹) для безразмерных величин
...
- Эмитирует тип маршрута (TMC/XYK) для прозрачности
...
`Модель пола (два разных ориентира)`:

```
k = R_native × R_foreign (инвариант постоянного произведения)

Спотовая цена пула:
P_xyk = R_foreign / R_native

Стресс-огибающая пола для объёма распродажи x:
P_stress(x) = k / (R_native + x)²
```

`Предположение референс-состояния для ratio-оценок`:

Чтобы выразить стресс-пол как долю потолка только через `s` и `a`, используется нормировка в паритетном референс-состоянии:
...
| Сценарий     | Источник продаж (Source)         | Доля продаж (`s`) | Отношение стресс-пол/потолок | Волатильность |
| :----------- | :------------------------------- | :---------------- | :--------------------------- | :------------ |
...
- `Выход Users`: Представляет полную продажу начального публичного предложения. При поддержке Корзины_A (`a=33%`) стресс-пол удерживается на уровне 25% от референс-потолка.
- `Выход System`: Представляет катастрофический сценарий, где казначейские корзины (B, C, D) входят в оборот и также продаются. При соблюдении предположений протокола Корзина_A подразумевает приблизительно 11% моделируемый коэффициент стресс-пола в этом сценарии.
...
Когда сжигание уменьшает циркулирующее предложение (`S_circ`) на `ΔS`, улучшается стресс-огибающая, потому что сжимается максимум потенциальной распродажи.

Обозначим sell-pressure как `x_max = λ · S_circ`, где `λ ∈ (0, 1]` зависит от governance и рыночного режима.

1.  `Сжатие предложения`: `S'_circ = S_circ - ΔS`
2.  `Повышение стресс-пола`: уменьшение `x_max` поднимает стресс-огибающую.
    ```
...
3.  `Снижение потолка`: цена минтинга снижается при сокращении предложения.
    ...
    ```
4.  `Замечание про спот`: при неизменных резервах `P_xyk = R_foreign/R_native` не меняется
5.  `Результат`: стресс-коридор сжимается с обеих сторон (Двунаправленная компрессия).

`Скорость повышения стресс-пола (при фиксированных k, R_native, λ на dt)`:

```
dP_stress,max/dt ∝ (скорость_сжигания × λ × k) / (R_native + λ·S_circ)³

Свойства:
- Скорость пропорциональна скорости сжигания (управление контролирует через комиссии)
- Скорость возрастает по мере сжатия sellable inventory
- Зависимость нелинейна из-за кубического знаменателя
```
...
P_xyk = const(R_native, R_foreign)
       (спотовая цена пула неизменна при постоянных резервах)
P_stress,max = k / (R_native + λ·S_circ)²
             (стресс-пол растёт при сжатии sellable inventory)

Чистый эффект: сжатие стресс-коридора со сходимостью пол-потолок
```

`Пример прогрессии` (R_foreign = 666 667 Foreign, m = 1 500 000):

| Предложение | P_ceiling | P_stress (мин) | Спред |
| ----------- | --------- | -------------- | ----- |
| 1M          | 1,501     | 0,11           | 13,6× |
| 500k        | 0,751     | 0,22           | 3,4×  |
| 200k        | 0,301     | 0,56           | 0,54× |

`Критическая точка`: Когда `P_stress > P_ceiling`, арбитражные стимулы меняются. Эмиссия становится более привлекательной, чем рыночная продажа, создавая естественное равновесие.
...
Точка мгновенного ценового паритета, где кривая и XYK дают одинаковую предельную цену.
...
- `P_backing` — канонический ориентир обеспечения в этом документе (`P · S ≈ R_foreign`)
- `P_parity` — ориентир паритета механизмов для маршрутизации и исполнения
- Эти ориентиры могут расходиться в конкретном состоянии и сходиться через рыночную активность и сжигание
...
- `Поведение роутера`: Ниже `P_backing` предложение "перепродано" (сильная поддержка пола). Выше `P_backing` предложение имеет "премию" (обусловлено полезностью).

`Численный пример`:

```
R_foreign = 1 000 000 иностранных токенов
...
`Интерпретация`: `P_backing` — это уровень обеспечения, где подразумеваемая кривой рыночная капитализация равна foreign-резерву. Он отличается от мгновенного паритета механизмов (`P_parity`), при этом динамика сходимости по-прежнему зависит от скорости сжигания.

`Зависимость от управления`: `P_backing` явно зависит от параметров `R_foreign` и `m`. Изменения в распределении TOL или наклоне напрямую изменяют этот целевой уровень обеспечения. Это позволяет управлению корректировать долгосрочные ценовые ориентиры через модификацию параметров.
...
- `Близость к полу`: Сниженные арбитражные возможности; ожидание катализатора
...
`Динамика волатильности`: Сжатие стресс-спреда (`P_ceiling - P_stress,max`) монотонно уменьшается со временем при устойчивом сжигании. Это представляет математическое следствие сжатия предложения с фиксированными коэффициентами резервов, а не экономическое обещание, но детерминированный результат, зависящий от поддержания управлением условий.
...
- Цена сходится к √(R_foreign × m / PRECISION)
...
- PPB = 10⁹ для безразмерных отношений
...
`Защитное право вето команды`: После удаления супер-пользователя команда сохраняет `право вето`, представляющее Native токены, содержащиеся в резервах TOL. Этот механизм вето обеспечивает постоянную защиту от атак управления, одновременно обеспечивая прогрессивную децентрализацию.
...
`Вестинг права вето`: Система поддерживает постепенную децентрализацию через механизмы вестинга, применяемые к праву вето. По мере созревания протокола и демонстрации компетентности сообществом управления, право вето может быть постепенно сокращено:

- `Фаза 1`: Полное право вето команды с силой голоса TOL
- `Фаза 2`: Сниженные пороги вето (например, 75% → 60% → 51%)
- `Фаза 3`: Вето с таймлоком и периодами охлаждения
...
Спот XYK:                  P_xyk = R_foreign / R_native
Стресс-пол:                P_stress(x) = k / (R_native + x)²
Равновесие обеспечения:    P_backing ≈ √(R_foreign × m / PRECISION)
Паритетное равновесие:     P_parity = R_foreign / R_native
Скорость стресс-пола:      dP_stress,max/dt ∝ (скорость_сжигания × λ × k)/(R_native + λ·S_circ)³
```
