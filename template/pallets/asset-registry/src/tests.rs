...
fn register_foreign_asset_works() {
  new_test_ext().execute_with(|| {
    ...
    assert_eq!(AssetRegistry::location_to_asset(&location), Some(1000));

    // 3. Verify Event
    System::assert_last_event(RuntimeEvent::AssetRegistry(Event::ForeignAssetRegistered {
      asset_id: 1000,
...
fn register_duplicate_fails() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      AssetRegistry::register_foreign_asset(RuntimeOrigin::root(), location, metadata, 10, true),
...
fn register_foreign_asset_fails_bad_origin() {
  new_test_ext().execute_with(|| {
    ...
    assert_noop!(
      AssetRegistry::register_foreign_asset(RuntimeOrigin::signed(2), location, metadata, 10, true),
...
fn migrate_location_key_emits_event() {
  new_test_ext().execute_with(|| {
    ...
    let asset_id = AssetRegistry::location_to_asset(&old_location).unwrap();
    ...
    System::assert_last_event(RuntimeEvent::AssetRegistry(Event::MigrationApplied {
      asset_id,
      old_location,
      new_location,
    }));
