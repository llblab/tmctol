...
use polkadot_sdk::frame_support::{
  ...
  traits::{ConstU128, ConstU32, ConstU64, Currency, ExistenceRequirement, Get},
  PalletId,
};
...
thread_local! {
    ...
    // TMC Rates: Asset -> Native Amount per 1 Unit of Asset
    // Logic: Mint X Native for 1 Foreign
    pub static TMC_RATES: RefCell<BTreeMap<AssetKind, u128>> = const { RefCell::new(BTreeMap::new()) };
...
pub fn set_tmc_rate(asset: AssetKind, rate: u128) {
  TMC_RATES.with(|r| {
    r.borrow_mut().insert(asset, rate);
  });
}
...
impl pallet_axial_router::types::TmcInterface<u64, u128> for MockTmcPallet {
  ...
  fn calculate_user_receives(
    ...
    let rate = TMC_RATES
      .with(|r| r.borrow().get(&foreign_asset).cloned())
  ...
  fn mint_with_distribution(
    ...
    let amount_out = Self::calculate_user_receives(foreign_asset, foreign_amount)?;
    ...
    // 2. Mint Native asset to user
    <Balances as FungibleMutate<u64>>::mint_into(who, amount_out)?;

    Ok(amount_out)
  }
  fn do_burn_internal(_token_asset: AssetKind, _amount: u128) -> Result<(), DispatchError> {
    Ok(())
  }
  fn burn_tokens(_: AssetKind, _: u128) -> Result<(), DispatchError> {
    Ok(())
  }
}
...
impl pallet_axial_router::types::AssetConversionApi<u64, u128> for MockAssetConversionAdapter {
  ...
  fn swap_exact_tokens_for_tokens(
    ...
    // Support only direct swap in mock for simplicity
    if path.len() > 2 {
      return Err(DispatchError::Other("Multi-hop mock not implemented"));
    }
    ...
    // 2. Update State (Pools)
    let pool_id =
      Self::get_pool_id(asset_in, asset_out).ok_or(DispatchError::Other("Pool missing"))?;
    POOLS.with(|p| {
      let mut pools = p.borrow_mut();
      let (res_a, res_b) = pools.get(&pool_id).cloned().unwrap();
      let (new_res_a, new_res_b) = if pool_id.0 == asset_in {
        (
    ...
    match asset_in {
      ...
      AssetKind::Local(id) | AssetKind::Foreign(id) => {
        // Burn from user, Mint to Pool (12345)
        <Assets as FungiblesMutate<u64>>::burn_from(
          id,
          &who,
          amount_in,
        ...
        <Assets as FungiblesMutate<u64>>::mint_into(id, &12345, amount_in)?;
      }
    }

    // Mint/Give to recipient
    match asset_out {
      AssetKind::Native => {
        <Balances as FungibleMutate<u64>>::mint_into(&recipient, amount_out)?;
      }
      AssetKind::Local(id) | AssetKind::Foreign(id) => {
        <Assets as FungiblesMutate<u64>>::mint_into(id, &recipient, amount_out)?;
      }
    }
...
impl Get<Permill> for RouterFeeStub {
  fn get() -> Permill {
    Permill::from_rational(20u32, 10000u32) // 0.2%
  }
}
