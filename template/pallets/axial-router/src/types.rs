...
pub trait TmcInterface<AccountId, Balance> {
  ...
  /// Burn tokens for supply reduction (secure internal call without Origin)
  fn do_burn_internal(token_asset: AssetKind, amount: Balance) -> Result<(), DispatchError>;

  /// Burn tokens for supply reduction (legacy - requires signed origin)
  /// Note: This requires signed origin and cannot be called directly from pallet logic
  fn burn_tokens(token_asset: AssetKind, amount: Balance) -> Result<(), DispatchError>;
}
