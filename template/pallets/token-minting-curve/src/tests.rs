...
mod tests {
  ...
  fn large_number_stress_test() {
    ...
    new_test_ext().execute_with(|| {
      ...
      let curve_config = crate::CurveConfig {
        initial_price,
        slope,
        current_supply: 1_000_000u128,
        total_minted: 1_000_000u128,
        foreign_asset,
  ...
  fn quadratic_integration_verification() {
    ...
    new_test_ext().execute_with(|| {
      ...
      polkadot_sdk::frame_support::assert_ok!(crate::Pallet::<Test>::create_curve(
        crate::mock::RuntimeOrigin::signed(user),
        asset_id,
        foreign_asset,
        initial_price,
        slope,
      ...
      assert_eq!(curve.current_supply, 0);

      // 5. Verify Calculus Implementation
      // Verify that the pallet calculates the correct output amount for the calculated cost.

      // First, manually advance supply to s0 to setup correct state for verifying delta
      if s0 > 0 {
        let mut curve = crate::Pallet::<Test>::get_curve(asset_id).expect("Curve should exist");
        curve.current_supply = s0;
        curve.total_minted = s0;
        crate::TokenCurves::<Test>::insert(asset_id, curve);
      }
  ...
  #[test]
  fn governance_can_pause_and_unpause_minting() {
    use crate::mock::{new_test_ext, RuntimeOrigin, TokenMintingCurve};
    use polkadot_sdk::frame_support::assert_ok;

    new_test_ext().execute_with(|| {
      // Initially minting should not be paused
      assert!(!TokenMintingCurve::minting_paused());

      // Pause minting as root
      assert_ok!(TokenMintingCurve::pause_minting(RuntimeOrigin::root()));
      assert!(TokenMintingCurve::minting_paused());

      // Unpause minting as root
  ...
  #[test]
  fn only_governance_can_pause_minting() {
    use crate::mock::{new_test_ext, RuntimeOrigin, TokenMintingCurve};
    use polkadot_sdk::frame_support::assert_noop;
    use polkadot_sdk::sp_runtime::DispatchError;

    new_test_ext().execute_with(|| {
      // Regular user cannot pause minting
      assert_noop!(
        TokenMintingCurve::pause_minting(RuntimeOrigin::signed(1)),
        DispatchError::BadOrigin
      );

      // Regular user cannot unpause minting
      assert_noop!(
        TokenMintingCurve::unpause_minting(RuntimeOrigin::signed(1)),
        DispatchError::BadOrigin
      );
    });
  }
  ...
  #[test]
  fn minting_fails_when_paused() {
    use crate::mock::{new_test_ext, Assets, RuntimeOrigin, Test, TokenMintingCurve};
    ...
    new_test_ext().execute_with(|| {
      ...
      // Use asset IDs that don't conflict with genesis (1 and 2 are pre-created)
      let asset_id = AssetKind::Local(10);
      let foreign_asset = AssetKind::Local(11);

      // Set up foreign asset first
      assert_ok!(Assets::force_create(RuntimeOrigin::root(), 11, 1, true, 1));
      ...
      // Create curve with PRECISION-relative parameters
      ...
      // Create the native asset that will be minted
      assert_ok!(Assets::force_create(RuntimeOrigin::root(), 10, 1, true, 1));

      // Minting should fail when paused
      assert_noop!(
        TokenMintingCurve::mint_tokens(RuntimeOrigin::signed(1), asset_id, 10 * PRECISION),
        crate::Error::<Test>::MintingIsPaused
      );
  ...
  fn governance_can_update_curve_parameters() {
    use crate::mock::{new_test_ext, RuntimeOrigin, Test, TokenMintingCurve};
    use crate::types::AssetKind;
    use polkadot_sdk::frame_support::assert_ok;
