...
pub mod params {
  ...
  /// in addition to the 0.3% AMM fee from AssetConversion pools.
  pub const AXIAL_ROUTER_FEE: Permill = Permill::from_parts(5_000); // 50 bps = 5000 ppm

  /// TMC curve slope parameter (0.0001 per token).
  ///
  ...
  pub const TMC_SLOPE_PARAMETER: Balance = PRECISION / 10_000; // 0.0001 in PRECISION units

  /// TMC burn percentage for bidirectional compression (20%).
  pub const TMC_BURN_RATIO: Permill = Permill::from_percent(20);
