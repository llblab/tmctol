...
use super::common::{
  add_liquidity, create_pool, create_test_asset, mint_tokens, new_test_ext, ALICE, ASSET_A,
  ASSET_FOREIGN, BOB, CHARLIE, MINT_AMOUNT,
};
use crate::{
  configs::AssetKind, AccountId, AssetConversion, Assets, Balance, Balances, RuntimeOrigin, System,
  EXISTENTIAL_DEPOSIT,
};
...
fn test_token_swap_high_minimum_output_fails() {
  new_test_ext().execute_with(|| {
    ...
    assert!(swap_exact_tokens_for_tokens(
      RuntimeOrigin::signed(trader.clone()),
      vec![local_asset, native_asset],
      swap_amount,
